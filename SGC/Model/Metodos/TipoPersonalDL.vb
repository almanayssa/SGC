...
#Region "Select"
        ...
        Public Function ListarDemandaTipoPersonal(ByVal fecIni As Date, ByVal fecFin As Date, id_comite As String, id_tipo As String) As System.Collections.Generic.List(Of Entidades.TipoPersonalBE) Implements Interfaces.ITipoPersonal.ListarDemandaTipoPersonal
            Dim oListadoTipoPersonal As New List(Of TipoPersonalBE)
            Dim oTipoPersonal As TipoPersonalBE
            Dim strConn As String = ConfigurationManager.ConnectionStrings("SGC").ConnectionString
            Dim sqlConn As New SqlConnection(strConn)
            Dim sqlCmd As New SqlCommand("comite.SP_LISTAR_TIPOPERSONAL_DEMANDA", sqlConn)
            Dim dr As SqlDataReader = Nothing
            sqlCmd.CommandType = CommandType.StoredProcedure
            ...
            Try
                ...
                While dr.Read()
                    oTipoPersonal = New TipoPersonalBE
                    oTipoPersonal.id_tipo_personal = dr("id_tipo_personal")
                    oTipoPersonal.descripcion = dr("descripcion")
                    oTipoPersonal.cantidad = dr("cant")
                    oTipoPersonal.monto = dr("monto")
                    oListadoTipoPersonal.Add(oTipoPersonal)
                End While
                dr.Close()
                Return oListadoTipoPersonal
